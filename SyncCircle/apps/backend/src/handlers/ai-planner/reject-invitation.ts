/**
 * POST /meeting-invitations/{invitationId}/reject — Reject a meeting invitation.
 *
 ...
 * service to reject the invitation (updating status and triggering session rollup).
 *
 * Error mapping:
...
import { rejectInvitation } from '../../services/planning-session.service.js';
...
export async function handler(
  ...
  try {
    ...
    const result = await rejectInvitation(userId, invitationId);
    ...
    logger.error('Unexpected error in rejectInvitation handler', {
