 ...
 *
 * Requests new proposed time options, excluding all previously rejected slots.
 *
...
import { nextOption } from '../../services/planning-session.service.js';
...
export async function handler(
  ...
  try {
    ...
    // 3. Call service (no body needed for next-option)
    const result = await nextOption(userId, sessionId);
    ...
    logger.error('Unexpected error in next-option handler', {
