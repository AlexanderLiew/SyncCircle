...
</html>`}async function k(e){let{recipientEmail:t,senderDisplayName:n,token:r}=e,l=ue(r),m=`${n} wants to connect with you on ${d}`,E=pe(n,l),g=me(n,l);if(le.toLowerCase()==="local")return c.info("Local email adapter: invitation email composed",{recipient:t,subject:m,plainTextBody:E,htmlBody:g,invitationLink:l}),{emailSent:!0};try{let u=ce(),a=new oe({Source:ae,Destination:{ToAddresses:[t]},Message:{Subject:{Data:m,Charset:"UTF-8"},Body:{Text:{Data:E,Charset:"UTF-8"},Html:{Data:g,Charset:"UTF-8"}}}});return await u.send(a),c.info("Invitation email sent successfully",{recipient:t}),{emailSent:!0}}catch(u){let a=u instanceof Error?u.message:String(u);return c.error("Failed to send invitation email via SES",{errorMessage:a,recipient:t}),{emailSent:!1}}}import{DynamoDBClient as Ee}from"@aws-sdk/client-dynamodb";import{DynamoDBDocumentClient as ge,PutCommand as tt,GetCommand as nt,QueryCommand as fe,BatchGetCommand as rt,UpdateCommand as st}from"@aws-sdk/lib-dynamodb";var Re=new Ee({}),Ie=ge.from(Re),ye=process.env.USER_PROFILES_TABLE??"UserProfiles",Ae="normalizedEmail-index";async function O(e){return(await Ie.send(new fe({TableName:ye,IndexName:Ae,KeyConditionExpression:"normalizedEmail = :email",ExpressionAttributeValues:{":email":e},Limit:1}))).Items?.[0]}import{DynamoDBDocumentClient as Ne,PutCommand as Se,GetCommand as ot,QueryCommand as q,UpdateCommand as at}from"@aws-sdk/lib-dynamodb";import{DynamoDBClient as he}from"@aws-sdk/client-dynamodb";var Te=new he({}),T=Ne.from(Te),_=process.env.FRIEND_REQUESTS_TABLE;async function H(e){await T.send(new Se({TableName:_,Item:e}))}async function M(e,t){let[n,r]=await Promise.all([T.send(new q({TableName:_,IndexName:"senderUserId-createdAt-index",KeyConditionExpression:"senderUserId = :sender",FilterExpression:"receiverUserId = :receiver AND #status = :pending",ExpressionAttributeValues:{":sender":e,":receiver":t,":pending":p.PENDING},ExpressionAttributeNames:{"#status":"status"}})),T.send(new q({TableName:_,IndexName:"senderUserId-createdAt-index",KeyConditionExpression:"senderUserId = :sender",FilterExpression:"receiverUserId = :receiver AND #status = :pending",ExpressionAttributeValues:{":sender":t,":receiver":e,":pending":p.PENDING},ExpressionAttributeNames:{"#status":"status"}}))]);return[...n.Items??[],...r.Items??[]]}import{DynamoDBDocumentClient as De,PutCommand as ut,QueryCommand as we,UpdateCommand as pt,TransactWriteCommand as mt}from"@aws-sdk/lib-dynamodb";import{DynamoDBClient as be}from"@aws-sdk/client-dynamodb";var ve=new be({}),Ce=De.from(ve),Pe=process.env.FRIENDSHIPS_TABLE,gt=process.env.FRIEND_REQUESTS_TABLE;async function B(e,t){return(await Ce.send(new we({TableName:Pe,IndexName:"userIdLow-index",KeyConditionExpression:"userIdLow = :low",FilterExpression:"userIdHigh = :high",ExpressionAttributeValues:{":low":e,":high":t}}))).Items?.[0]}function V(e,t){return e<t?{userIdLow:e,userIdHigh:t}:{userIdLow:t,userIdHigh:e}}var G={"Content-Type":"application/json","Access-Control-Allow-Origin":"*","Access-Control-Allow-Headers":"Content-Type,Authorization","Access-Control-Allow-Methods":"GET,POST,PUT,DELETE,OPTIONS"};function K(e){return{statusCode:201,headers:G,body:JSON.stringify(e)}}function o(e,t,n,r){let l={error:n,code:t,...r!==void 0&&{field:r}};return{statusCode:e,headers:G,body:JSON.stringify(l)}}async function _t(e){try{let t=e.requestContext.authorizer?.claims;if(!t)return o(401,i.UNAUTHORIZED,"Missing authentication");let n=t.sub,r=t.email;if(!e.body)return o(400,i.VALIDATION_ERROR,"Request body is required");let l;try{l=JSON.parse(e.body)}catch{return o(400,i.VALIDATION_ERROR,"Invalid JSON body")}let{email:m,displayName:E,recipientEmail:g,recipientDisplayName:u}=l,a=m||g,A=E||u,D=C(a);if(!D.valid){let s=D.errors[0];return o(400,i.VALIDATION_ERROR,s.message,s.field)}let w=P(A);if(!w.valid){let s=w.errors[0];return o(400,i.VALIDATION_ERROR,s.message,s.field)}if(F(r,a))return o(400,i.SELF_REQUEST,"Cannot send a friend request to yourself");let b=I(a),f=await O(b);if(f){let{userIdLow:s,userIdHigh:X}=V(n,f.userId),v=await B(s,X);if(v&&v.status===N.ACTIVE)return o(409,i.ALREADY_FRIENDS,"You are already friends with this user");if((await M(n,f.userId)).length>0)return o(409,i.PENDING_EXISTS,"A pending friend request already exists between you and this user")}let{token:z,tokenHash:$,expiresAt:Q}=U(),Y=new Date().toISOString(),R=Ue(),j={requestId:R,senderUserId:n,receiverUserId:f?.userId??"UNREGISTERED",receiverEmail:a,normalizedReceiverEmail:b,senderDisplayName:A,status:p.PENDING,tokenHash:$,tokenExpiresAt:Q,createdAt:Y};await H(j);let x=!1;try{x=(await k({recipientEmail:a,senderDisplayName:A,token:z})).emailSent}catch(s){c.error("Failed to send invitation email",{errorMessage:s instanceof Error?s.message:String(s),requestId:R}),x=!1}return c.info("Friend request created successfully",{requestId:R}),K({requestId:R,status:p.PENDING,emailSent:x})}catch(t){return c.error("Unexpected error in createFriendRequest handler",{errorMessage:t instanceof Error?t.message:String(t)}),o(500,i.INTERNAL_ERROR,"An unexpected error occurred")}}export{_t as handler};
